...
import {
  IMembersInformationsServiceToken,
  IMembersInformationsService,
} from './interfaces/member-information.interface';
...
export class MembersInformationsController {
  constructor(
    @Inject(IMembersInformationsServiceToken)
    private readonly membersInformationsService: IMembersInformationsService,
  ) {}
  ...
  @ApiResponse({
    status: 201,
    description: 'Les informations ont été créées avec succès.',
  ...
  @Get(':uuid')
  @ApiOperation({
    summary: "Récupérer les informations d'un membre par son UUID",
  })
  @ApiResponse({
    status: 200,
    description: 'Les informations ont été trouvées.',
    type: MemberInformation,
  })
  ...
  async update(
    ...
    @Body() updateMemberInformationsDto: UpdateMemberInformationsDto,
  ) {
    const memberInformations = await this.membersInformationsService.update(
      uuid,
      updateMemberInformationsDto,
    );
    if (!memberInformations) {
      throw new NotFoundException(
        `MemberInformations with UUID "${uuid}" not found`,
      );
    }
