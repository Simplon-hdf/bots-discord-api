...
describe('AuthService', () => {
  ...
  describe('getGuildMember', () => {
    it('should get guild member successfully', async () => {
      ...
      mockHttpService.get.mockReturnValue(of({ data: mockMember }));

      // Act
      const result = await service.getGuildMember(accessToken, guildId, userId);
    ...
    it('should throw UnauthorizedException when user is not a member of the guild', async () => {
      ...
      // Act & Assert
      await expect(service.getGuildMember(accessToken, guildId, userId)).rejects.toThrow(
        new UnauthorizedException('User is not a member of the specified guild'),
      );
    });
    ...
    it('should throw UnauthorizedException when getting guild member fails', async () => {
      ...
      // Act & Assert
      await expect(service.getGuildMember(accessToken, guildId, userId)).rejects.toThrow(
        new UnauthorizedException('Failed to get guild member information'),
      );
    });
  });
  ...
  describe('validateUserGuild', () => {
    it('should validate user guild successfully when user is in guild', async () => {
      ...
      // Assert
      expect(result).toEqual({ isValid: true, roles: ['role_id_1', 'role_id_2'] });
      expect(service.getUserGuilds).toHaveBeenCalledWith(accessToken);
      expect(service.getGuildMember).toHaveBeenCalledWith(accessToken, 'guild_id', userId);
    });
    ...
    it('should return isValid false when user is not in allowed guild', async () => {
      ...
      // Assert
      expect(result).toEqual({ isValid: false, roles: [] });
      expect(service.getUserGuilds).toHaveBeenCalledWith(accessToken);
